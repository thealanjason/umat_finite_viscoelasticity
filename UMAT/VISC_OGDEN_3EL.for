...
!     BeOLD_3       - VISCOUS LEFT CAUCHY-GREEN DEFORMATION TENSOR (AT N-1)
!     BeOLDINV_3    - INVERSE(BeOLD) 
!     CiOLD_3       - VISCOUS RIGHT CAUCHY-GREEN DEFORMATION TENSOR (AT N-1)
!     CiOLDINV_3    - INVERSE(CiOLD)
!     BeTR_3        - NEQ TRIAL LEFT CAUCHY-GREEN TENSOR
!     BeTR__3       - COPY OF BeTR FOR EIGEN VALUE\VECTOR DECOMPOSITION 
!     Be_3          - NEQ LEFT CAUCHY-GREEN TENSOR 
!     TAUNEQ_3      - NEQ KIRCHOFF STRESS TENSOR
!     PVBeTR_3      - EIGEN VALUES OF BeTR
!     PDBeTR_3      - EIGEN VECTORS OF BeTR
!     EPSeTR_3      - TRIAL NEQ PRINCIPAL(EIGEN) STRAIN
!     PVBe_3        - EIGEN VALUES OF Be 
!     EPSe_3        - NEQ LOGARITHMIC PRINCIPAL(EIGEN) STRAIN 
!     PVBeBAR_3     - EIGEN VALUES OF NEQ DEVIATORIC LEFT-CAUCHY GREEN TENSOR
!     DEVTAU_3      - EIGEN VALUES OF DEVIATORIC NEQ KIRCHOFF STRESS TENSOR
!     RESVEC_3      - RESIDUAL VECTOR FOR NEWTON RAPHSON'S ITERATION
!     DDEVTAUDEPSe_3- D(DEVTAU) / D(EPSe)
!     KMAT_3        - K MATRIX FOR NEWTON RAPHSON'S ITERATION
!     KINV_3        - INVERSE OF K MATRIX
!     DELEPSe_3     - DELTA EPSe FOR NEWTON RAPHSON'S ITERATION 
!     PVTAU_3       - EIGEN VALUES OF TAUNEQ
!     PDTAU_3       - EIGEN VECTORS OF TAUNEQ
!     DPVTAUDEPSe_3 - D(PVTAU) / D(EPSe)
!     CALG_3        - NEQ ALGORITHMIC TANGENT MODULUS
!     SIGMANEQ_3    - NEQ CAUCHY STRESS TENSOR
!     L4NEQ_3       - NEQ INTERMEDIATE MATERIAL TANGENT STIFFNESS MODULUS
!     C4NEQ_3       - NEQ SPATIAL TANGENT STIFFNESS MODULUS
!     C4NEQJ_3      - NEQ SPATIAL TANGENT STIFFNESS MODULUS (FE)
!     CNEQ_3        - NEQ SPATIAL TANGENT STIFFNESS MODULUS (VOIGT)
!       
...
! ---------------------------------------------------------------------
      ...
      DOUBLE PRECISION, DIMENSION(3, 3) :: BeOLD_3
      DOUBLE PRECISION, DIMENSION(3, 3) :: BeOLDINV_3
      DOUBLE PRECISION, DIMENSION(3, 3) :: CiOLD_3
      DOUBLE PRECISION, DIMENSION(3, 3) :: CiOLDINV_3
      DOUBLE PRECISION, DIMENSION(3, 3) :: BeTR_3
      DOUBLE PRECISION, DIMENSION(3, 3) :: BeTR__3
      DOUBLE PRECISION, DIMENSION(3, 3) :: Be_3
      DOUBLE PRECISION, DIMENSION(3,3) :: TAUNEQ_3
      DOUBLE PRECISION, DIMENSION(3) :: PVBeTR_3
      DOUBLE PRECISION, DIMENSION(3,3) :: PDBeTR_3
      DOUBLE PRECISION, DIMENSION(3) :: EPSeTR_3
      DOUBLE PRECISION, DIMENSION(3) :: PVBe_3
      DOUBLE PRECISION, DIMENSION(3) :: EPSe_3
      DOUBLE PRECISION, DIMENSION(3) :: PVBeBAR_3
      DOUBLE PRECISION, DIMENSION(3) :: DEVTAU_3
      DOUBLE PRECISION, DIMENSION(3) :: RESVEC_3
      DOUBLE PRECISION, DIMENSION(3,3) :: DDEVTAUDEPSe_3
      DOUBLE PRECISION, DIMENSION(3,3) :: KMAT_3
      DOUBLE PRECISION, DIMENSION(3,3) :: KINV_3
      DOUBLE PRECISION, DIMENSION(3) :: DELEPSe_3
      DOUBLE PRECISION, DIMENSION(3) :: PVTAU_3
      DOUBLE PRECISION, DIMENSION(3,3) :: PDTAU_3
      DOUBLE PRECISION, DIMENSION(3,3) :: DPVTAUDEPSe_3
      DOUBLE PRECISION, DIMENSION(3,3) :: CALG_3
      DOUBLE PRECISION, DIMENSION(3,3) :: SIGMANEQ_3
      DOUBLE PRECISION, DIMENSION(3,3,3,3) :: L4NEQ_3
      DOUBLE PRECISION, DIMENSION(3,3,3,3) :: C4NEQ_3
      DOUBLE PRECISION, DIMENSION(3,3,3,3) :: C4NEQJ_3
      DOUBLE PRECISION, DIMENSION(6,6) :: CNEQ_3
...
!
!     ##### THIRD VISCOUS ELEMENT
!     Je_3       - JACOBIAN OF NEQ DEFORMATION GRADIENT
!     NORMRES_3  - NORM OF RESIDUAL VECTOR
! ---------------------------------------------------------------------
      ...
      DOUBLE PRECISION MUVIS_3
      DOUBLE PRECISION ALPHAVIS_3
      DOUBLE PRECISION KVIS_3
      DOUBLE PRECISION ETADEV_3
      DOUBLE PRECISION ETAVOL_3
      DOUBLE PRECISION Je_3
      DOUBLE PRECISION NORMRES_3
...
!     ##### THIRD VISCOUS ELEMENT    
!
      CALL M33INV(BeOLD_3, BeOLDINV_3, OK_FLAG)
      CiOLD_3 = MATMUL(TRANSPOSE(DFGRD0), MATMUL(BeOLDINV_3, DFGRD0))
      CALL M33INV(CiOLD_3, CiOLDINV_3, OK_FLAG)
      BeTR_3  = MATMUL(DFGRD1, MATMUL(CiOLDINV_3, TRANSPOSE(DFGRD1)))
!
! ---------------------------------------------------------------------
...
!     GET EIGEN VALUES AND EIGEN VECTORS OF BeTR
      DO I = 1,3
        DO J = 1,3
          BeTR__3(I,J) = BeTR(I,J)
        END DO
      END DO
      CALL DSYEVJ3(BeTR__3, PDBeTR_3, PVBeTR_3)
      PDBeTR_3 = TRANSPOSE(PDBeTR_3)
!
!     CALCULATE EPSeTR
      DO I = 1,3
        EPSeTR_3(I)=ONE/TWO*LOG(PVBeTR_3(I))
      END DO
!
!     PERFORM LOCAL ITERATION
!     1. Initialize Iteration Variables
...
!       - Calculating Deviatoric Principal Values of Be
        PVBeBAR_3(1) = Je_3**(-TWO/THREE)*PVBe_3(1)
        PVBeBAR_3(2) = Je_3**(-TWO/THREE)*PVBe_3(2)
        PVBeBAR_3(3) = Je_3**(-TWO/THREE)*PVBe_3(3)
!       - Calculating Principal Values of Deviatoric Kirchoff Stress
        DEVTAU_3(1) = MUVIS_3 *
     1                ((TWO/THREE)*(PVBeBAR_3(1)**(ALPHAVIS_3/TWO))
     2                -(ONE/THREE)*(PVBeBAR_3(2)**(ALPHAVIS_3/TWO))
     3                -(ONE/THREE)*(PVBeBAR_3(3)**(ALPHAVIS_3/TWO)))
        DEVTAU_3(2) = MUVIS_3 *
     1                ((TWO/THREE)*(PVBeBAR_3(2)**(ALPHAVIS_3/TWO))
     2                -(ONE/THREE)*(PVBeBAR_3(3)**(ALPHAVIS_3/TWO))
     3                -(ONE/THREE)*(PVBeBAR_3(1)**(ALPHAVIS_3/TWO)))
        DEVTAU_3(3) = MUVIS_3 *
     1                ((TWO/THREE)*(PVBeBAR_3(3)**(ALPHAVIS_3/TWO))
     2                -(ONE/THREE)*(PVBeBAR_3(1)**(ALPHAVIS_3/TWO))
     3                -(ONE/THREE)*(PVBeBAR_3(2)**(ALPHAVIS_3/TWO)))
!       - Calculating Residual Vector
        DO N = 1, 3
...
!       - Calculating Norm of Residual Vector
     ...
     1             + RESVEC_3(2)**TWO
     2             + RESVEC_3(3)**TWO)**(ONE/TWO)
!       - Terminate if Norm of Residual Vector is less than Tolerance
        IF ((ITER.GT.1).AND.ABS(NORMRES_3).LT.RESTOL) THEN
            EXIT
        END IF
!       - Calculating dDEVTAU/dEPSe
        DDEVTAUDEPSe_3(1,1) = MUVIS_3 * ALPHAVIS_3 
     1                   *((FOUR/NINE)*(PVBeBAR_3(1)**(ALPHAVIS_3/TWO))
     2                     +(ONE/NINE)*(PVBeBAR_3(2)**(ALPHAVIS_3/TWO)
     3                                 +PVBeBAR_3(3)**(ALPHAVIS_3/TWO)))
        DDEVTAUDEPSe_3(2,2) = MUVIS_3 * ALPHAVIS_3 
     1                   *((FOUR/NINE)*(PVBeBAR_3(2)**(ALPHAVIS_3/TWO))
     2                     +(ONE/NINE)*(PVBeBAR_3(3)**(ALPHAVIS_3/TWO)
     3                                 +PVBeBAR_3(1)**(ALPHAVIS_3/TWO))) 
        DDEVTAUDEPSe_3(3,3) = MUVIS_3 * ALPHAVIS_3 
     1                   *((FOUR/NINE)*(PVBeBAR_3(3)**(ALPHAVIS_3/TWO))
     2                     +(ONE/NINE)*(PVBeBAR_3(1)**(ALPHAVIS_3/TWO)
     3                                 +PVBeBAR_3(2)**(ALPHAVIS_3/TWO)))
        DDEVTAUDEPSe_3(1,2) = MUVIS_3 * ALPHAVIS_3 
     1                   *((-TWO/NINE)*(PVBeBAR_3(1)**(ALPHAVIS_3/TWO)
     2                                 +PVBeBAR_3(2)**(ALPHAVIS_3/TWO))
     3                     +(ONE/NINE)*(PVBeBAR_3(3)**(ALPHAVIS_3/TWO)))
        DDEVTAUDEPSe_3(1,3) = MUVIS_3 * ALPHAVIS_3 
     1                   *((-TWO/NINE)*(PVBeBAR_3(1)**(ALPHAVIS_3/TWO)
     2                                 +PVBeBAR_3(3)**(ALPHAVIS_3/TWO))
     3                     +(ONE/NINE)*(PVBeBAR_3(2)**(ALPHAVIS_3/TWO)))
        DDEVTAUDEPSe_3(2,3) = MUVIS_3 * ALPHAVIS_3 
     1                   *((-TWO/NINE)*(PVBeBAR_3(2)**(ALPHAVIS_3/TWO)
     2                                 +PVBeBAR_3(3)**(ALPHAVIS_3/TWO))
     3                     +(ONE/NINE)*(PVBeBAR_3(1)**(ALPHAVIS_3/TWO)))
...
!       - Calculating K Matrix for Newton Rhapson
     ...
     2                +(ONE/(THREE*ETAVOL_3)*KVIS_3*Je_3*Je_3))
        ...
        END DO
!       - Calculating KINV
        CALL M33INV(KMAT_3, KINV_3, OK_FLAG)
!       - Calculating NEQ Strain Increment
        DELEPSe_3(1) = -(KINV_3(1,1)*RESVEC_3(1)
     1                  +KINV_3(1,2)*RESVEC_3(2)
     2                  +KINV_3(1,3)*RESVEC_3(3))
                      
        DELEPSe_3(2) = -(KINV_3(2,1)*RESVEC_3(1)
     1                  +KINV_3(2,2)*RESVEC_3(2)
     2                  +KINV_3(2,3)*RESVEC_3(3))
        
        DELEPSe_3(3) = -(KINV_3(3,1)*RESVEC_3(1)
     1                  +KINV_3(3,2)*RESVEC_3(2)
     2                  +KINV_3(3,3)*RESVEC_3(3))
!       - Calculating Updated NEQ Strain
        DO I = 1,3
        EPSe_3(I) = EPSe_3(I) + DELEPSe_3(I)
        END DO
!       - Updating Eigen Value of Be
        DO I = 1,3
          PVBe_3(I) = EXP(TWO*EPSe_3(I))
        END DO
      END DO
!     3. Non-Covergence Test for Newton Rhapson
      IF (ITER.GT.200) THEN
...
!     1. Be
      DO I=1,3
        DO J=1,3
            Be_3(J,I)=PVBe_3(1)*PDBeTR_3(1,J)*PDBeTR_3(1,I)
     1               +PVBe_3(2)*PDBeTR_3(2,J)*PDBeTR_3(2,I)
     2               +PVBe_3(3)*PDBeTR_3(3,J)*PDBeTR_3(3,I)
        END DO
      END DO
! 
!     2. PVTAU
      DO I=1,3
            PVTAU_3(I) = DEVTAU_3(I) + KVIS_3/TWO*(Je_3*Je_3 - ONE)
      END DO
! 
!     3. PDTAU
      DO I=1,3
        DO J=1,3
          PDTAU_3(J,I) = PDBeTR_3(J,I)
        END DO
      END DO
! 
!     4. TAUNEQ
      DO I=1,3
        DO J=1,3
          TAUNEQ_3(J,I)=PVTAU_3(1)*PDTAU_3(1,J)*PDTAU_3(1,I)
     1                 +PVTAU_3(2)*PDTAU_3(2,J)*PDTAU_3(2,I)
     2                 +PVTAU_3(3)*PDTAU_3(3,J)*PDTAU_3(3,I)
        END DO
      END DO
! 
!     5. CALG
!     - Calculating Calculating dPVTAU/dEPSe
      DO I=1,3
        DO J=1,3
          DPVTAUDEPSe_3(J,I) = DDEVTAUDEPSe_3(J,I) + KVIS_3*Je_3*Je_3
        END DO
      END DO
!     - Calculating CALG
      CALG_3 = MATMUL(DPVTAUDEPSe_3, KINV_3)
!
! ---------------------------------------------------------------------
...
!     CALCULATE NEQ CAUCHY STRESS TENSOR
      ...
      DO J=1,3
        DO K=1,3
          SIGMANEQ_3(J,K) = TAUNEQ_3(J,K)/DET
        END DO
      END DO
!
!     CALCULATE MATERIAL TENSOR (L4NEQ) IN INTERMEDIATE CONFIGURATION 
...
!     2. CALCULATE COEFFICIENTS FOR ABAB / ABBA
      DO J=1,3
        DO K=1,3
          ...
          END IF
!         - In case of Equal PVBeTR, Use Alternate Form
          IF (ABS(PVBeTR_3(J)-PVBeTR_3(K)).LT.EPS) THEN
            L4NEQ_3(J,K,J,K) = (L4NEQ_3(J,J,J,J)-L4NEQ_3(J,J,K,K))/TWO
          ELSE
            L4NEQ_3(J,K,J,K) = ((PVTAU_3(K)/PVBeTR_3(K))
     1                         -(PVTAU_3(J)/PVBeTR_3(J)))
     1                        /(PVBeTR(K)-PVBeTR(J))
          END IF
      ...
      DO J=1,3
        DO K=1,3
          DO L=1,3
            DO M=1,3
              C4NEQ_3(J,K,L,M) = 
     1          L4NEQ_3(1,1,1,1)*PVBeTR_3(1)*PVBeTR_3(1)
     1                          *PDBeTR_3(1,J)*PDBeTR_3(1,K)
     1                          *PDBeTR_3(1,L)*PDBeTR_3(1,M)
     1         +L4NEQ_3(1,1,2,2)*PVBeTR_3(1)*PVBeTR_3(2)
     1                          *PDBeTR_3(1,J)*PDBeTR_3(1,K)
     1                          *PDBeTR_3(2,L)*PDBeTR_3(2,M)
     1         +L4NEQ_3(1,1,3,3)*PVBeTR_3(1)*PVBeTR_3(3)
     1                          *PDBeTR_3(1,J)*PDBeTR_3(1,K)
     1                          *PDBeTR_3(3,L)*PDBeTR_3(3,M)
     1         +L4NEQ_3(2,2,1,1)*PVBeTR_3(2)*PVBeTR_3(1)
     1                          *PDBeTR_3(2,J)*PDBeTR_3(2,K)
     1                          *PDBeTR_3(1,L)*PDBeTR_3(1,M)
     1         +L4NEQ_3(2,2,2,2)*PVBeTR_3(2)*PVBeTR_3(2)
     1                          *PDBeTR_3(2,J)*PDBeTR_3(2,K)
     1                          *PDBeTR_3(2,L)*PDBeTR_3(2,M)
     1         +L4NEQ_3(2,2,3,3)*PVBeTR_3(2)*PVBeTR_3(3)
     1                          *PDBeTR_3(2,J)*PDBeTR_3(2,K)
     1                          *PDBeTR_3(3,L)*PDBeTR_3(3,M)
     1         +L4NEQ_3(3,3,1,1)*PVBeTR_3(3)*PVBeTR_3(1)
     1                          *PDBeTR_3(3,J)*PDBeTR_3(3,K)
     1                          *PDBeTR_3(1,L)*PDBeTR_3(1,M)
     1         +L4NEQ_3(3,3,2,2)*PVBeTR_3(3)*PVBeTR_3(2)
     1                          *PDBeTR_3(3,J)*PDBeTR_3(3,K)
     1                          *PDBeTR_3(2,L)*PDBeTR_3(2,M)
     1         +L4NEQ_3(3,3,3,3)*PVBeTR_3(3)*PVBeTR_3(3)
     1                          *PDBeTR_3(3,J)*PDBeTR_3(3,K)
     1                          *PDBeTR_3(3,L)*PDBeTR_3(3,M)
     1         +L4NEQ_3(1,2,1,2)*PVBeTR_3(1)*PVBeTR_3(2)
     1                          *PDBeTR_3(1,J)*PDBeTR_3(2,K)
     1                          *PDBeTR_3(1,L)*PDBeTR_3(2,M)
     1         +L4NEQ_3(1,2,2,1)*PVBeTR_3(1)*PVBeTR_3(2)
     1                          *PDBeTR_3(1,J)*PDBeTR_3(2,K)
     1                          *PDBeTR_3(2,L)*PDBeTR_3(1,M)
     1         +L4NEQ_3(1,3,1,3)*PVBeTR_3(1)*PVBeTR_3(3)
     1                          *PDBeTR_3(1,J)*PDBeTR_3(3,K)
     1                          *PDBeTR_3(1,L)*PDBeTR_3(3,M)
     1         +L4NEQ_3(1,3,3,1)*PVBeTR_3(1)*PVBeTR_3(3)
     1                          *PDBeTR_3(1,J)*PDBeTR_3(3,K)
     1                          *PDBeTR_3(3,L)*PDBeTR_3(1,M)
     1         +L4NEQ_3(2,1,2,1)*PVBeTR_3(2)*PVBeTR_3(1)
     1                          *PDBeTR_3(2,J)*PDBeTR_3(1,K)
     1                          *PDBeTR_3(2,L)*PDBeTR_3(1,M)
     1         +L4NEQ_3(2,1,1,2)*PVBeTR_3(2)*PVBeTR_3(1)
     1                          *PDBeTR_3(2,J)*PDBeTR_3(1,K)
     1                          *PDBeTR_3(1,L)*PDBeTR_3(2,M)
     1         +L4NEQ_3(2,3,2,3)*PVBeTR_3(2)*PVBeTR_3(3)
     1                          *PDBeTR_3(2,J)*PDBeTR_3(3,K)
     1                          *PDBeTR_3(2,L)*PDBeTR_3(3,M)
     1         +L4NEQ_3(2,3,3,2)*PVBeTR_3(2)*PVBeTR_3(3)
     1                          *PDBeTR_3(2,J)*PDBeTR_3(3,K)
     1                          *PDBeTR_3(3,L)*PDBeTR_3(2,M)
     1         +L4NEQ_3(3,1,3,1)*PVBeTR_3(3)*PVBeTR_3(1)
     1                          *PDBeTR_3(3,J)*PDBeTR_3(1,K)
     1                          *PDBeTR_3(3,L)*PDBeTR_3(1,M)
     1         +L4NEQ_3(3,1,1,3)*PVBeTR_3(3)*PVBeTR_3(1)
     1                          *PDBeTR_3(3,J)*PDBeTR_3(1,K)
     1                          *PDBeTR_3(1,L)*PDBeTR_3(3,M)
     1         +L4NEQ_3(3,2,3,2)*PVBeTR_3(3)*PVBeTR_3(2)
     1                          *PDBeTR_3(3,J)*PDBeTR_3(2,K)
     1                          *PDBeTR_3(3,L)*PDBeTR_3(2,M)
     1         +L4NEQ_3(3,2,2,3)*PVBeTR_3(3)*PVBeTR_3(2)
     1                          *PDBeTR_3(3,J)*PDBeTR_3(2,K)
     1                          *PDBeTR_3(2,L)*PDBeTR_3(3,M)
            END DO
          END DO
...
!     CALCLUATE JAUMANN RATE FORM OF MATERIAL TENSOR
     ...
     1                        +(IDT2(I,K)*TAUNEQ_3(J,L)
     1                         +IDT2(I,L)*TAUNEQ_3(J,K)
     1                         +IDT2(J,K)*TAUNEQ_3(I,L)
     1                         +IDT2(J,L)*TAUNEQ_3(I,K))/(TWO*DET)        
            END DO
          END DO
        END DO
      END DO
...
!     GENERATE ABAQUS NEQ TANGENT STIFFNESS MATRIX (VOIGT NOTATION)
      CNEQ_3 = ZERO
      CNEQ_3(1,1) = C4NEQJ_3(1,1,1,1)
      CNEQ_3(1,2) = C4NEQJ_3(1,1,2,2)
      CNEQ_3(1,3) = C4NEQJ_3(1,1,3,3)
      CNEQ_3(1,4) = C4NEQJ_3(1,1,1,2)
      CNEQ_3(1,5) = C4NEQJ_3(1,1,1,3)
      CNEQ_3(1,6) = C4NEQJ_3(1,1,2,3)
      CNEQ_3(2,1) = CNEQ_3(1,2)
      CNEQ_3(2,2) = C4NEQJ_3(2,2,2,2)
      CNEQ_3(2,3) = C4NEQJ_3(2,2,3,3)
      CNEQ_3(2,4) = C4NEQJ_3(2,2,1,2)
      CNEQ_3(2,5) = C4NEQJ_3(2,2,1,3)
      CNEQ_3(2,6) = C4NEQJ_3(2,2,2,3)
      CNEQ_3(3,1) = CNEQ_3(1,3)
      CNEQ_3(3,2) = CNEQ_3(2,3)
      CNEQ_3(3,3) = C4NEQJ_3(3,3,3,3)
      CNEQ_3(3,4) = C4NEQJ_3(3,3,1,2)
      CNEQ_3(3,5) = C4NEQJ_3(3,3,1,3)
      CNEQ_3(3,6) = C4NEQJ_3(3,3,2,3)
      CNEQ_3(4,1) = CNEQ_3(1,4)
      CNEQ_3(4,2) = CNEQ_3(2,4)
      CNEQ_3(4,3) = CNEQ_3(3,4)
      CNEQ_3(4,4) = C4NEQJ_3(1,2,1,2)
      CNEQ_3(4,5) = C4NEQJ_3(1,2,1,3)
      CNEQ_3(4,6) = C4NEQJ_3(1,2,2,3)
      CNEQ_3(5,1) = CNEQ_3(1,5)
      CNEQ_3(5,2) = CNEQ_3(2,5)
      CNEQ_3(5,3) = CNEQ_3(3,5)
      CNEQ_3(5,4) = CNEQ_3(4,5)
      CNEQ_3(5,5) = C4NEQJ_3(1,3,1,3)
      CNEQ_3(5,6) = C4NEQJ_3(1,3,2,3)
      CNEQ_3(6,1) = CNEQ_3(1,6)
      CNEQ_3(6,2) = CNEQ_3(2,6)
      CNEQ_3(6,3) = CNEQ_3(3,6)
      CNEQ_3(6,4) = CNEQ_3(4,6)
      CNEQ_3(6,5) = CNEQ_3(5,6)
      CNEQ_3(6,6) = C4NEQJ_3(2,3,2,3)
! ---------------------------------------------------------------------
...
!     CALCULATE TOTAL ABAQUS TANGENT STIFNESS MATRIX
      DO J=1,6
        DO K=1,6
          DDSDDE(J,K) = CNEQ(J,K) + CNEQ_2(J,K)+ CNEQ_3(J,K)+ CEQ(J,K)
        END DO
      END DO
